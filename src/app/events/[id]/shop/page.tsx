...
export default function ShopDetailPage() {
  ...
  const fetchEvent = async () => {
    ...
    try {
      ...
      if (snap.exists()) setEvent({ id: snap.id, ...snap.data() } as ShopEvent);
    } catch (err) {
      console.error("Error fetching shop event:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-purple-50/20 flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-5">
          <span className="text-xs font-medium px-2.5 py-1 bg-pink-50 text-pink-600 rounded-full">
            ショップ巡り
          </span>
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            ...
            {event.meetingPlace && (
              <div className="bg-gray-50 rounded-xl p-3">
                <p className="text-xs text-gray-400 mb-1">集合場所</p>
                <p className="text-sm font-semibold text-gray-800">{event.meetingPlace}</p>
              </div>
            )}
          </div>

          {event.prefectures?.length ? (
            <p className="text-sm text-gray-600 mb-2">📍 {event.prefectures.join(", ")}</p>
          ) : null}
          {event.shops?.length ? (
            <p className="text-sm text-gray-600 mb-2">🏬 めぐる店: {event.shops.join(", ")}</p>
          ) : null}
          {event.items?.length ? (
            <p className="text-sm text-gray-600 mb-2">🎒 持ち物: {event.items.join(", ")}</p>
          ) : null}
          {event.description && (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-5">
          {!isOwner ? (
            ...
            <div className="flex gap-3">
              <Link href={`/events/${event.id}/edit/shop`} className="flex-1">
