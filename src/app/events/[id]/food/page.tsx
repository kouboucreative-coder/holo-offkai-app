...
export default function FoodDetailPage() {
  ...
  const fetchEvent = async () => {
    ...
    try {
      ...
      if (snap.exists()) setEvent({ id: snap.id, ...snap.data() } as FoodEvent);
    } catch (err) {
      console.error("Error fetching food event:", err);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            ...
            {event.prefecture && (
              <div className="bg-gray-50 rounded-xl p-3">
                ...
                <p className="text-sm font-semibold text-gray-800">{event.prefecture}</p>
              </div>
            )}
          </div>

          {event.foodShop && <p className="text-sm text-gray-600 mb-2">🍽️ {event.foodShop}</p>}
          {event.address && <p className="text-sm text-gray-600 mb-2">📍 {event.address}</p>}
          {event.items && <p className="text-sm text-gray-600 mb-2">🎒 持ち物: {event.items}</p>}
          {event.description && (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-5">
          {!isOwner ? (
            ...
            <div className="flex gap-3">
              <Link href={`/events/${event.id}/edit/food`} className="flex-1">
