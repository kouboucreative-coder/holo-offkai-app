...
export default function FoodEditPage() {
  ...
  const [items, setItems] = useState(""); // 持ち物や支払い方法
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      ...
      try {
        ...
        console.error("Error loading food event:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      alert("飲食イベントを更新しました！");
      router.push(`/events/${id}/food`);
    } catch (err) {
      console.error("Error updating food event:", err);
      alert("更新に失敗しました");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-200 via-green-100 to-green-300 flex flex-col">
      <Navbar />
      <main className="flex-1 p-6 max-w-2xl mx-auto w-full">
        <h1 className="text-3xl font-bold text-green-700 mb-6">🍽️ 飲食オフ会を編集</h1>
        ...
        <div className="bg-white/90 p-6 rounded-lg shadow space-y-4">
          ...
          {/* 飲食店名 */}
          <label className="block text-gray-800">飲食店名</label>
          ...
          <div className="flex gap-2 mt-6">
            ...
            <button onClick={() => router.push(`/events/${id}/food`)}
