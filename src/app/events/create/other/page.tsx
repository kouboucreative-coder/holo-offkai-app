...
export default function OtherCreatePage() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-purple-50/20 flex flex-col">
      ...
      <main className="flex-1 px-4 py-8 max-w-2xl mx-auto w-full">
        ...
        <div className="flex items-center gap-3 mb-6">
          <span className="text-xs font-medium px-2.5 py-1 bg-gray-100 text-gray-600 rounded-full">その他</span>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="イベントの内容を詳しく説明してください" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>会場名 *</label>
            <input type="text" value={venue} onChange={(e) => setVenue(e.target.value)} placeholder="例: ○○公園、△△センター" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>住所 *</label>
            <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="例: 神奈川県横浜市○○区1-2-3" className={inputCls} />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={handleSubmit} className="flex-1 py-3 bg-gray-700 text-white rounded-xl font-bold hover:bg-gray-800 hover:-translate-y-0.5 transition-all shadow-sm">
              作成する
            </button>
