...
export default function ViewingCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert("🎬 観賞会イベントを作成しました！");
      router.push("/events");
    } catch (err) {
      console.error("Error creating viewing event:", err);
      alert("イベント作成に失敗しました");
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-purple-50/20 flex flex-col">
      ...
      <main className="flex-1 px-4 py-8 max-w-2xl mx-auto w-full">
        ...
        <div className="flex items-center gap-3 mb-6">
          <span className="text-xs font-medium px-2.5 py-1 bg-rose-50 text-rose-600 rounded-full">観賞会</span>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
          ...
          <div>
            <label className={labelCls}>観賞コンテンツ *</label>
            <input type="text" value={content} onChange={(e) => setContent(e.target.value)} placeholder="例: ○○ライブBlu-ray、△△1期" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>会場名 *</label>
            <input type="text" value={venue} onChange={(e) => setVenue(e.target.value)} placeholder="例: ○○さんの家、△△レンタルスペース" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>住所 *</label>
            <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="例: 大阪府大阪市○○区1-2-3" className={inputCls} />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={handleSubmit} className="flex-1 py-3 bg-rose-500 text-white rounded-xl font-bold hover:bg-rose-600 hover:-translate-y-0.5 transition-all shadow-sm">
              作成する
            </button>
