...
export default function KaraokeCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert("🎤 カラオケイベントを作成しました！");
      router.push("/events");
    } catch (err) {
      console.error("Error creating karaoke event:", err);
      alert("イベント作成に失敗しました");
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-purple-50/20 flex flex-col">
      ...
      <main className="flex-1 px-4 py-8 max-w-2xl mx-auto w-full">
        ...
        <div className="flex items-center gap-3 mb-6">
          <span className="text-xs font-medium px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-full">カラオケ</span>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
          ...
          <div>
            <label className={labelCls}>カラオケ店名 *</label>
            <input type="text" value={karaokeShop} onChange={(e) => setKaraokeShop(e.target.value)} placeholder="例: カラオケ館 ○○店" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>住所 *</label>
            <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="例: 名古屋市中村区○○1-2-3" className={inputCls} />
          </div>
          ...
          <div>
            <label className={labelCls}>支払い方法</label>
