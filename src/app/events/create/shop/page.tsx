"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { db, auth } from "@/lib/firebase";
import { addDoc, collection, serverTimestamp } from "firebase/firestore";
import Navbar from "@/components/Navbar";
import Link from "next/link";

const PREFECTURES = [
  "北海道","青森県","岩手県","宮城県","秋田県","山形県","福島県",
  "茨城県","栃木県","群馬県","埼玉県","千葉県","東京都","神奈川県",
  "新潟県","富山県","石川県","福井県","山梨県","長野県",
  "岐阜県","静岡県","愛知県","三重県",
  "滋賀県","京都府","大阪府","兵庫県","奈良県","和歌山県",
  "鳥取県","島根県","岡山県","広島県","山口県",
  "徳島県","香川県","愛媛県","高知県",
  "福岡県","佐賀県","長崎県","熊本県","大分県","宮崎県","鹿児島県","沖縄県"
];

export default function CreateShopEventPage() {
  const router = useRouter();
  const user = auth.currentUser;

  const [title, setTitle] = useState("");
  const [date, setDate] = useState("");
  const [timeStart, setTimeStart] = useState("");
  const [timeEnd, setTimeEnd] = useState("");
  const [capacity, setCapacity] = useState(2);
  const [description, setDescription] = useState("");
  const [items, setItems] = useState("");
  const [meetingPlace, setMeetingPlace] = useState("");
  const [shops, setShops] = useState<string[]>([""]);
  const [prefectures, setPrefectures] = useState<string[]>([""]);

  const changeCapacity = (diff: number) => {
    setCapacity((prev) => Math.max(2, prev + diff));
  };

  const addShop = () => setShops([...shops, ""]);
  const updateShop = (i: number, value: string) => {
    const next = [...shops];
    next[i] = value;
    setShops(next);
  };
  const removeShop = (i: number) => setShops(shops.filter((_, idx) => idx !== i));

  const addPrefecture = () => setPrefectures([...prefectures, ""]);
  const updatePrefecture = (i: number, value: string) => {
    const next = [...prefectures];
    next[i] = value;
    setPrefectures(next);
  };
  const removePrefecture = (i: number) =>
    setPrefectures(prefectures.filter((_, idx) => idx !== i));

  const handleSubmit = async () => {
    if (!user) {
      alert("ログインしてください");
      return;
    }
    if (
      !title.trim() ||
      !date.trim() ||
      !timeStart.trim() ||
      !timeEnd.trim() ||
      !description.trim() ||
      !items.trim() ||
      !meetingPlace.trim() ||
      shops.some((s) => !s.trim()) ||
      prefectures.some((p) => !p.trim())
    ) {
      alert("未入力の必須項目があります。");
      return;
    }
    try {
      await addDoc(collection(db, "events"), {
        genre: "shop",
        title,
        date,
        timeStart,
        timeEnd,
        description,
        capacity,
        items: [items],
        meetingPlace,
        shops,
        prefectures,
        createdBy: user.uid,
        createdByName: user.displayName || "名無し",
        participants: [],
        createdAt: serverTimestamp(),
        surveyCompleted: false,
      });
      alert("🏬 ショップ巡りイベントを作成しました！");
      router.push("/events");
    } catch (e) {
      console.error(e);
      alert("イベント作成に失敗しました");
    }
  };

  const inputCls = "w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-400 text-gray-800 text-sm";
  const labelCls = "block text-sm font-medium text-gray-700 mb-1";

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-purple-50/20 flex flex-col">
      <Navbar />
      <main className="flex-1 px-4 py-8 max-w-2xl mx-auto w-full">
        <Link href="/events/create" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-6 transition">
          ← ジャンル選択に戻る
        </Link>

        <div className="flex items-center gap-3 mb-6">
          <span className="text-xs font-medium px-2.5 py-1 bg-pink-50 text-pink-600 rounded-full">ショップ巡り</span>
          <h1 className="text-2xl font-bold text-gray-900">イベントを作成</h1>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
          <div>
            <label className={labelCls}>タイトル *</label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>日付 *</label>
            <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>開始・終了時間 *</label>
            <div className="flex gap-2">
              <input type="time" value={timeStart} onChange={(e) => setTimeStart(e.target.value)} className={inputCls} />
              <input type="time" value={timeEnd} onChange={(e) => setTimeEnd(e.target.value)} className={inputCls} />
            </div>
          </div>

          <div>
            <label className={labelCls}>募集人数</label>
            <div className="flex items-center gap-2">
              <button onClick={() => changeCapacity(-10)} className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition">-10</button>
              <button onClick={() => changeCapacity(-1)} className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition">-1</button>
              <span className="px-5 py-2 bg-blue-50 text-blue-700 font-bold rounded-xl text-sm">{capacity}</span>
              <button onClick={() => changeCapacity(1)} className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition">+1</button>
              <button onClick={() => changeCapacity(10)} className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition">+10</button>
            </div>
          </div>

          <div>
            <label className={labelCls}>詳細説明 *</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>持ち物 *</label>
            <input type="text" value={items} onChange={(e) => setItems(e.target.value)} placeholder="例: 交通費、昼飯代など必須なもの" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>集合場所 *</label>
            <input type="text" value={meetingPlace} onChange={(e) => setMeetingPlace(e.target.value)} placeholder="例: 名古屋駅○○口前" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>めぐる店 *</label>
            {shops.map((shop, idx) => (
              <div key={idx} className="flex gap-2 mb-2">
                <input type="text" value={shop} onChange={(e) => updateShop(idx, e.target.value)} className={inputCls} />
                <button onClick={() => removeShop(idx)} className="px-3 py-2 border border-red-200 text-red-500 rounded-xl text-sm hover:bg-red-50 transition">削除</button>
              </div>
            ))}
            <button onClick={addShop} className="px-4 py-2 border border-blue-200 text-blue-600 rounded-xl text-sm font-medium hover:bg-blue-50 transition">
              ＋ 店を追加
            </button>
          </div>

          <div>
            <label className={labelCls}>都道府県 *</label>
            {prefectures.map((pref, idx) => (
              <div key={idx} className="flex gap-2 mb-2">
                <select value={pref} onChange={(e) => updatePrefecture(idx, e.target.value)} className={inputCls}>
                  <option value="">選択してください</option>
                  {PREFECTURES.map((p) => <option key={p} value={p}>{p}</option>)}
                </select>
                <button onClick={() => removePrefecture(idx)} className="px-3 py-2 border border-red-200 text-red-500 rounded-xl text-sm hover:bg-red-50 transition">削除</button>
              </div>
            ))}
            <button onClick={addPrefecture} className="px-4 py-2 border border-blue-200 text-blue-600 rounded-xl text-sm font-medium hover:bg-blue-50 transition">
              ＋ 都道府県を追加
            </button>
          </div>

          <div className="flex gap-3 pt-2">
            <button onClick={handleSubmit} className="flex-1 py-3 bg-orange-500 text-white rounded-xl font-bold hover:bg-orange-600 hover:-translate-y-0.5 transition-all shadow-sm">
              作成する
            </button>
            <button onClick={() => router.push("/events/create")} className="flex-1 py-3 border border-gray-200 text-gray-600 rounded-xl font-semibold hover:bg-gray-50 transition">
              キャンセル
            </button>
          </div>
        </div>
      </main>
    </div>
  );
}
